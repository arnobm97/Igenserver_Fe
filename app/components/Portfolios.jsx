...
const pages = Array(4).fill({
  component: Portfolio,
  title: "Page",
  transform: "perspective(1200px) translate3d(0px, -62px, 150px) rotateX(-45deg) scale3d(0.83, 0.76, 1)",
...
const calculateTransformAndClipPath = (scrollPercentage) => {
 const clampedScroll = Math.min(scrollPercentage, 0.8) / 0.8;
  ...
  const translateZ = interpolate(150, 0, clampedScroll); // Reduced from 190 to 150
  const rotateX = interpolate(-45, 0, clampedScroll);   // Changed from -56.5 to -45
  const scaleX = interpolate(0.53, 1, clampedScroll);   // Kept original scaling
...
const Page = ({ pageIndex, updateTransform, page }) => {
  ...
  return (
    <motion.div
      ref={pageRef}
      className="sticky top-5 lg:top-7 left-24 w-full md:w-[calc(100%-100px)] xl:w-[calc(100%-100px)] grid place-content-center border border-[rgba(255,255,255,0.18)] rounded-2xl mx-auto overflow-hidden"
      style={{
  ...
  return (
    ...
    <ScrollMouseIndicator />
      ...
      <button
        ...
        className="hidden fixed top-12 left-5 text-orange-500 h-14 w-14 md:grid place-content-center rounded-full border-[0.5px] border-[#DB6E27] z-50"
        aria-label="Undo"
      >
      ...
      <div className="relative text-lg font-extrabold h-[30vh] xl:h-[550px] w-full grid place-content-center text-center overflow-hidden">
